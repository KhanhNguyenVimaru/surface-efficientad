...
def plot_metrics_chart(metrics_dict, output_path):
    ...
    ax.set_title("EfficientAD Capsule - Test Metrics", fontsize=14, fontweight="bold")
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    for bar, val in zip(bars, values):
        height = bar.get_height()
        ax.annotate(f"{val:.4f}", xy=(bar.get_x() + bar.get_width() / 2, height), xytext=(0, 3), textcoords="offset points", ha="center", va="bottom", fontsize=10, fontweight="bold")
    plt.tight_layout()
...
def plot_sample_visualizations(predictions, test_dataset, output_path, max_samples=8):
    ...
    for i in range(num_samples):
        ...
        pred = predictions[i]
        pred_score = to_scalar(_get_attr(pred, "pred_score"))
        pred_label = to_scalar(_get_attr(pred, "pred_label"))
        ...
        info = f"GT: {gt_text}\nPred: {pred_text}\nScore: {pred_score:.4f}" if pred_score is not None else f"GT: {gt_text}\nPred: {pred_text}"
        axes[i, 3].text(0.5, 0.5, info, ha="center", va="center", fontsize=11, bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
...
def main():
    ...
    scalar_metrics = {k: to_scalar(v) for k, v in result.items() if to_scalar(v) is not None}
    if scalar_metrics:
        plot_metrics_chart(scalar_metrics, args.output_dir / "metrics_chart.png")
    ...
    if raw_predictions:
        ...
        try:
            ...
            plot_sample_visualizations(predictions, test_dataset, args.output_dir / "sample_visualizations.png", max_samples=args.max_viz)
