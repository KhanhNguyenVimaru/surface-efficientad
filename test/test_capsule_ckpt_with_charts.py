...
def plot_sample_visualizations(predictions, test_dataset, output_path, max_samples=8):
    ...
    for i in range(num_samples):
        ...
        pred = predictions[i]
        pred_score = to_scalar(pred.get("pred_score") if isinstance(pred, dict) else getattr(pred, "pred_score", None))
        pred_label = to_scalar(pred.get("pred_label") if isinstance(pred, dict) else getattr(pred, "pred_label", None))
        anomaly_map = pred.get("anomaly_map") if isinstance(pred, dict) else getattr(pred, "anomaly_map", None)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate EfficientAD capsule checkpoint and export charts/visualizations."
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
